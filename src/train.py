"""Train YOLOv11 on the prepared MIDV-2020 dataset.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train YOLOv11 for KYC document detection")
    p.add_argument("--data", type=Path, required=True,
                   help="Path to data.yaml")
    p.add_argument("--model", default="yolo11s",
                   choices=["yolo11n", "yolo11s", "yolo11m", "yolo11l", "yolo11x"],
                   help="YOLOv11 variant")
    ...
    p.add_argument("--name", default="kyc_2_1",
...
def main() -> None:
    ...
    # Lazy import to keep --help fast and avoid forcing ultralytics for non-train tools
